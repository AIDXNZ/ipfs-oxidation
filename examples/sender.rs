...
async fn async_main() {
    ...
    let mut topic = gossipsub::IdentTopic::new("test-net");
    ...
    let _ = swarm.listen_on("/ip4/0.0.0.0/tcp/0".parse().unwrap()).unwrap();
            ...
            event = swarm.select_next_some() => {
                match event {
                    SwarmEvent::Behaviour(MyBehaviourEvent::Identify(identify::Event::Received {info, ..})) => {
                        println!("{:?}", info);
                        swarm.behaviour_mut().gossipsub.publish(topic.clone(), "Hello im here".as_bytes()).unwrap();
                    } 
